...
#ifdef HW
// just print out the bit representation of a Fp element
static void outputHW(const fp a)
{
    byte temp;
    for (int n = 0; n < UINTBIG_LIMBS; n++)
    {
        for (int k = 63; k >= 0; k--)
        {
            temp = (a.x.c[n] >> k) & 1;
...
#ifdef HW
                count = count + (-maskisogeny);

                // return the sequence of "target primes" for checking the workflow
                // not really required in the final version, but helpful
                printf("%ld, ", targetprime[i]);

                // if number of real isog equals the desired step
                // print the x value of the co-domain curve and quit (just in order to speed up the attack)
                if (count == li)
                {
                    printf("\n");
                    outputHW(A.x);

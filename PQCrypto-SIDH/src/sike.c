...
//    if (!EphemeralSecretAgreement_B_extended(sk + MSG_BYTES, ct, jinvariant_, 1) == 0) {
    if (!EphemeralSecretAgreement_B_extended(sk, ct, jinvariant_, 1) == 0) {
        goto Hashing;
    }
#ifdef DO_VALGRIND_CHECK
...
int crypto_kem_dec_leak(unsigned char *ss, const unsigned char *ct, const unsigned char *sk, const int iteration, int *bitseq)
{ // SIKE's decapsulation
  // Input:   secret key sk         (CRYPTO_SECRETKEYBYTES = MSG_BYTES + SECRETKEY_B_BYTES + CRYPTO_PUBLICKEYBYTES bytes)
  //          ciphertext message ct (CRYPTO_CIPHERTEXTBYTES = CRYPTO_PUBLICKEYBYTES + MSG_BYTES bytes) 
  // Outputs: shared secret ss      (CRYPTO_BYTES bytes)
    ...
    // Decrypt
    if (EphemeralSecretAgreement_B_extended_leak(sk + MSG_BYTES, ct, jinvariant_, 1, iteration, bitseq) == 0) {
         goto Hashing;
    }

    // // Decrypt
    // if (!EphemeralSecretAgreement_B_extended_leak(sk + MSG_BYTES, ct, jinvariant_, 1, iteration) == 0) {
    //     goto Hashing;
    // }
